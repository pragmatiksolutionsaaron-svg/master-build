...
Every row is accounted for. Always. This is a mathematical constraint enforced automatically.
...
Every SLM-generated function must pass a safety check. If it doesn't start with `lambda`, or contains `import`, `exec`, `eval`, or `os` — reject immediately and route the cluster to quarantine.
...
Every AI-applied transformation is logged: `[Row_ID, Old_Value, New_Value, Lambda_Applied, Confidence_Score, Model_Version, Timestamp]`.
...
def apply_fix_to_cluster(df: pd.DataFrame, column: str, fix: dict) -> pd.DataFrame:
    if fix['confidence_score'] < 0.75:
        ...
        return df
    # eval is safe here — output passed strict lambda-only validation gate above
...
def reconciliation_check(source: int, success: int, quarantine: int):
    """Mathematical zero-data-loss guarantee. Any mismatch > 0 is an immediate Sev-1."""
...
- **Defend the lambda rule**: "The AI suggests the fix. We execute it. We audit it. We can roll it back. Non-negotiable."
...
- **Zero silent data loss**: `Source == Success + Quarantine` holds on every batch run
